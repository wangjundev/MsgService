...
public class NotificationUtils extends ContextWrapper {
    ...
    public NotificationUtils(Context base) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            Log.i("Junwang", "NotificationUtils targetSDKVersion > 26");
     ...
     * @param content                   content
     */
    public void sendNotification(int notifyId, String title, String content , int icon, PendingIntent pendingIntent) {
        Notification build;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            //android 8.0以上需要特殊处理，也就是targetSDKVersion为26以上
            //通知用到NotificationCompat()这个V4库中的方法。但是在实际使用时发现书上的代码已经过时并且Android8.0已经不支持这种写法
