...
@ContentTag(type = MessageConstants.CONTENT_TYPE_SINGLE_CARD)
public class SingleCardMessageContent extends MessageContent {
    private String content;
    // 引用信息
//    private QuoteInfo quoteInfo;

    public SingleCardMessageContent() {
    }

    public SingleCardMessageContent(String content) {
...
//        dest.writeParcelable(this.quoteInfo, flags);
    ...
    protected SingleCardMessageContent(Parcel in) {
        super(in);
        this.content = in.readString();
//        this.quoteInfo = in.readParcelable(QuoteInfo.class.getClassLoader());
    }

    public static final Creator<SingleCardMessageContent> CREATOR = new Creator<SingleCardMessageContent>() {
        @Override
        public SingleCardMessageContent createFromParcel(Parcel source) {
            return new SingleCardMessageContent(source);
        }

        @Override
        public SingleCardMessageContent[] newArray(int size) {
            return new SingleCardMessageContent[size];
        }
    };
}
